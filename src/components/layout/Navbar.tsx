        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex-shrink-0 flex items-center justify-center flex-1 md:flex-none">
           ...
           {/* Desktop Navigation */}
            <div className="hidden md:flex flex-1 justify-center space-x-8 rtl:space-x-reverse items-center">
              {['nav.home', 'nav.category', 'nav.trending', 'nav.hot_offer'].map((key) => {
                const handleNavClick = (e: React.MouseEvent) => {
                  e.preventDefault();
                  if (key === 'nav.home') setCurrentView('home');
                  else if (key === 'nav.category') setCurrentView('catalog');
                  else if (key === 'nav.trending') setCurrentView('trending');
                  else if (key === 'nav.hot_offer') setCurrentView('hot_offer');
                };
                return (
          ...
          <div className="flex flex-1 justify-end items-center space-x-4 rtl:space-x-reverse">
            <button 
              ...
              className={`hover:text-primary hover:border-primary transition-colors flex items-center gap-1.5 text-xs uppercase font-serif tracking-[0.1em] border border-[#D946A6]/40 px-2.5 py-1.5 font-medium ${isScrolled ? 'text-foreground hover:bg-surface' : 'text-white hover:bg-white/10'}`}
              title={language === 'ar' ? 'تتبع طلبك' : 'Track Order'}
            >
              <Truck size={14} className="text-[#D946A6]" />
            ...
            <button className={`hover:text-primary transition-colors hidden sm:block ${isScrolled ? 'text-foreground' : 'text-white'}`}>
             ...
             <button 
               ...
               className={`hover:text-primary transition-colors relative ${isScrolled ? 'text-foreground' : 'text-white'}`}
             >
               <ShoppingBag size={20} />
             ...
             <button 
               className={`hidden lg:flex items-center gap-2 px-4 py-2 rounded-full text-[10px] uppercase tracking-widest font-bold transition-all duration-300 ${
                 isScrolled 
